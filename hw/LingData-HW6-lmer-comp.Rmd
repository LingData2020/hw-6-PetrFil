...
upsid %>%
  ...
  geom_point() +
  theme_bw() +
  geom_line(data = augment(fit2), aes(x = consonants, y = .fitted), color = "blue")
```
...
```
 ...
 _consonansts_ variable is still significant. p-value<0,05 but it has increased after we added _area_ random effect.We can see vertical lines and some of them are parallel because the additional constant(u(area)) changes the function.
...
upsid %>%
  ...
  geom_point() +
  theme_bw() +
  geom_line(data = augment(fit3), aes(x = consonants, y = .fitted), color = "blue")
```
...
```{3.3}
 _constants_ is not significant anymore (p-value>0,05). In comparison to the previous graph, the slope angle has changed as well as The length of some lines. It happens because "u(area)" element from the formula influences beta_1 and changes the function graph.
```
...
```{r 4.1, include=FALSE}
fit4 <- lmer(vowels ~ consonants + (0+consonants|area) + (1|area), upsid)
summary(fit4)
```
...
upsid %>%
  ...
  geom_point() +
  theme_bw() +
  geom_line(data = augment(fit4), aes(x = consonants, y = .fitted), color = "red")
```
...
```{4.3}
_constants_ is not significant. 
All three predictiona lines are based on different formulas. 
1) $$vowels = \beta_0 + \beta_1 \times consonants + u(area).$$ According to this formula, the area may influence the number of vowels
$$vowels = \beta_0 + (\beta_1 + u(area))\times consonants.$$ This formula implies that the area causes in which way the number of consonants may ifluence the number of vowels
$$vowels = \beta_0 + u_1(area) + (\beta_1 + u_2(area))\times consonants.$$ This formula means that the area may separately influence both factors. 
```
